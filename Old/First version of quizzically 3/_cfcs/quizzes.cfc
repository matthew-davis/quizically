<CFCOMPONENT HINT="dynamic component">
	...
	<CFFUNCTION NAME="getLocations" ACCESS="public" RETURNTYPE="query">
        ...
        <CFQUERY NAME="locationdetails" DATASOURCE="#THIS.dsn#">
        	...
        	SELECT *
                ...
                ORDER BY location_type DESC, location ASC

        </CFQUERY>

		<CFRETURN locationdetails />
    ...
    <!--- Quiz Number --->
	<CFFUNCTION NAME="getQuizNumbers" ACCESS="public" RETURNTYPE="query">

        <CFQUERY NAME="quiznumber" DATASOURCE="#THIS.dsn#">

        	SELECT IFNULL(SUM(CASE WHEN quiz_cancelled IS NULL AND quiz_finished IS NULL THEN 1 ELSE 0 END), 0) AS ongoing_number,
                   IFNULL(SUM(CASE WHEN quiz_cancelled IS NOT NULL OR quiz_finished IS NOT NULL THEN 1 ELSE 0 END), 0) AS historic_number

                FROM quizzes

                <CFIF THIS.username_hash NEQ ''>
                	WHERE added_by = <CFQUERYPARAM VALUE="#THIS.username_hash#" CFSQLTYPE="cf_sql_varchar" />
                </CFIF>
	...
	<CFFUNCTION NAME="getQuizDetails" ACCESS="public" RETURNTYPE="query">
        ...
        <CFQUERY NAME="quizdetails" DATASOURCE="#THIS.dsn#">
        	...
        	SELECT *,
            	...
            	FROM quizzes a
                		LEFT JOIN locations b ON a.location_UUID = b.location_UUID
                        LEFT JOIN users c ON a.quiz_winner = c.user_UUID
                        LEFT JOIN users_details d ON c.user_details_UUID = d.user_details_UUID
	...
	<CFFUNCTION NAME="addQuiz" ACCESS="public" RETURNTYPE="struct">
		...
		<CFTRY>
                ...
                <CFTRANSACTION>
					...
					<CFQUERY NAME="additem3" DATASOURCE="#THIS.dsn#">
						...
						SELECT quiz_code
							...
							WHERE quiz_code NOT IN (SELECT quiz_code FROM quizzes)
                    ...
                    <CFQUERY NAME="additem4" DATASOURCE="#THIS.dsn#">
                        ...
                        INSERT INTO quizzes (quiz_UUID, quiz_code, quiz_rounds, quiz_questions, location_UUID, quiz_round_1, quiz_round_2, quiz_round_3, quiz_round_4, quiz_round_5, quiz_round_6, quiz_created, added_by)

                            VALUES	(
                                    <CFQUERYPARAM VALUE="#quiz_UUID#" CFSQLTYPE="cf_sql_varchar">,
                                    <CFQUERYPARAM VALUE="#additem3.quiz_code#" CFSQLTYPE="cf_sql_varchar">,
                    ...
                    <CFQUERY NAME="additem5" DATASOURCE="#THIS.dsn#">
                        ...
                        INSERT INTO games (game_UUID, quiz_UUID, game_rounds_left, game_questions_left, game_status)
            ...
            <CFSET resultStruct.quizCode = #additem3.quiz_code# />
	...
	<CFFUNCTION NAME="getQuestion" ACCESS="public" RETURNTYPE="query">
        ...
        <CFQUERY NAME="quizdetails" DATASOURCE="#THIS.dsn#">
        	...
        	SELECT b.game_UUID,
                ...
                FROM quizzes a
                		LEFT JOIN games b ON a.quiz_UUID = b.quiz_UUID
        ...
        <CFQUERY NAME="checkquestion" DATASOURCE="#THIS.dsn#">
        	...
        	SELECT *
                ...
                AND question_date = (SELECT MAX(question_date) FROM games_questions WHERE quiz_UUID = <CFQUERYPARAM VALUE="#THIS.quiz_UUID#" CFSQLTYPE="cf_sql_varchar" />)
        ...
        <CFQUERY NAME="selectquestion" DATASOURCE="#THIS.dsn#">
        	...
        	SELECT a.question_UUID,
                ...
                FROM questions a
                		LEFT JOIN categories b ON a.category_UUID = b.category_UUID

                <CFIF (checkquestion.RECORDCOUNT EQ 0) OR (checkquestion.question_status EQ 'complete')>

                    WHERE a.question_UUID NOT IN (SELECT DISTINCT IFNULL(c.question_UUID, 'x') AS question_UUID FROM games_questions c WHERE c.quiz_UUID = <CFQUERYPARAM VALUE="#THIS.quiz_UUID#" CFSQLTYPE="cf_sql_varchar" />)
        ...
        <CFIF (checkquestion.RECORDCOUNT EQ 0) OR (checkquestion.question_status EQ 'complete')>
            ...
            <CFQUERY NAME="insertintoquestions" DATASOURCE="#THIS.dsn#">
                ...
                INSERT INTO games_questions (game_question_UUID, quiz_UUID, game_UUID, question_UUID, round_number, question_number, question_status, question_date)
	...
	<CFFUNCTION NAME="getQuestionNumber" ACCESS="public" RETURNTYPE="query">
        ...
        <CFQUERY NAME="getquizdetails" DATASOURCE="#THIS.dsn#">
        	...
        	SELECT a.quiz_rounds - b.game_rounds_left + 1 AS round_number,
                ...
                FROM quizzes a
                		LEFT JOIN games b ON a.quiz_UUID = b.quiz_UUID
                        LEFT JOIN categories c ON a.quiz_round_1 = c.category_UUID
                        LEFT JOIN categories d ON a.quiz_round_2 = d.category_UUID
                        LEFT JOIN categories e ON a.quiz_round_3 = e.category_UUID
                        LEFT JOIN categories f ON a.quiz_round_4 = f.category_UUID
                        LEFT JOIN categories g ON a.quiz_round_5 = g.category_UUID
                        LEFT JOIN categories h ON a.quiz_round_6 = h.category_UUID
	...
	<CFFUNCTION NAME="nextQuestion" ACCESS="remote" RETURNFORMAT="plain" RETURNTYPE="string">
        ...
        <CFQUERY NAME="checknumbers1" DATASOURCE="#dsn#">
            ...
            SELECT *
            	...
            	FROM quizzes a
                		INNER JOIN games b ON a.quiz_UUID = b.quiz_UUID
            ...
            <CFELSE>
				...
				<CFQUERY NAME="finishquiz1" DATASOURCE="#dsn#">
                    ...
                    SELECT CASE WHEN a.participant_team = '' OR a.participant_team IS NULL THEN
						...
						FROM participants a
								LEFT JOIN players b ON a.player_UUID = b.player_UUID
								LEFT JOIN players_details c ON b.player_details_UUID = c.player_details_UUID
        ...
        <CFQUERY NAME="questionlist" DATASOURCE="#THIS.dsn#">
        	...
        	SELECT *,
            	...
            	FROM games_questions a
                		LEFT JOIN questions b ON a.question_UUID = b.question_UUID
	...
	<CFFUNCTION NAME="getLatestQuizzes" ACCESS="public" RETURNTYPE="query">
        ...
        <CFQUERY NAME="quizlist" DATASOURCE="#THIS.dsn#" MAXROWS="4">
        	...
        	SELECT *,
            	   SUBSTRING(d.user_email, 1, LOCATE('@', d.user_email) - 1) AS email_name
