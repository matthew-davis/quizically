...
<CFCOMPONENT HINT="dynamic component">
			...
			<CFELSE>
		        ...
		        <CFQUERY NAME="questioncheck" DATASOURCE="#dsn#">
		        	...
		        	SELECT a.question_UUID
		        		...
		        		AND a.question_date = (SELECT MAX(question_date) FROM games_questions WHERE quiz_UUID = <CFQUERYPARAM VALUE="#quiz_UUID#" CFSQLTYPE="cf_sql_varchar" />)
						AND a.question_status = 'complete'
						AND a.question_UUID NOT IN (SELECT question_UUID FROM interactions WHERE participant_UUID = <CFQUERYPARAM VALUE="#playercheck.participant_UUID#" CFSQLTYPE="cf_sql_varchar" />)
	...
	<CFFUNCTION NAME="checkAnswer" ACCESS="remote" RETURNFORMAT="plain">
		...
		<CFQUERY NAME="checkanswer4" DATASOURCE="#dsn#">
        	...
        	INSERT INTO interactions (interaction_UUID, quiz_UUID, participant_UUID, question_UUID, interaction_score, interaction_time, interaction_answer, interaction_correct)
