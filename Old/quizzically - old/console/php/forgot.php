...
$email = $link->real_escape_string($_POST["forgot_email"]);
$sendemail = 0; 

$sql = "SELECT SUBSTRING(CLIENT_EMAIL, 1, LOCATE('@', CLIENT_EMAIL) - 1) AS NAME, CLIENT_EMAIL FROM A_CLIENT WHERE CLIENT_EMAIL = '$email' LIMIT 1";
$res = $link->query($sql);

if ($res->num_rows == 0) {
	$arr = array("forgot_success" => "no account");
...
$sql = "UPDATE A_CLIENT SET EXPIRE = '$now', RESET = '$forgot_key' WHERE CLIENT_EMAIL = '$email'";
$res = $link->query($sql);
	
$mailer = "<div style='margin: 0 auto; width: 520px; padding: 12px; float: left;'>
			   <div>
		           <img src='http://www.quizzically.co.uk/quizzically/console/img/email_logo.png' alt='quizzically logo' style='float: left;' />
				   <br><br>
			   </div>
			   <br><br>
		       Hi " . $name . ",
		       ...
		       <br><br>
		       <a href='http://www.quizzically.co.uk/quizzically/console/php/change.php?forgot_key=" . $forgot_key . "'>http://www.quizzically.co.uk/quizzically/console/php/change.php?forgot_key=" . $forgot_key . "</a>
		       <br><br>
...
if(!$mail->Send()) {
	$arr = array("forgot_success" => "no send");
	echo json_encode($arr);
} else {
	$arr = array("forgot_success" => "forgot success");
	echo json_encode($arr);
}
