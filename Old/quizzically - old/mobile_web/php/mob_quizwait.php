...
//Get QUIZ_ID And QUESTION_LEFT
$sql = "SELECT QUIZ_ID, IFNULL(NO_OF_QUESTIONS, 0) AS NO_OF_QUESTIONS FROM B_QUIZ WHERE QUIZ_CODE = '$quizcode'";
$res = $link->query($sql);
		...
		if ($questionid != 0) {
			...
			if ($status == "R") {
				$arr = array("mob_quizwait_success" => "mob quizwait go", 
							 ...
							 "mob_quizwait_number" => $questionsleft,
