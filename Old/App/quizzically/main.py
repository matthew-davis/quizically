...
### This particular service oerforms two tasks:
### - Store and retrieve tag-value pairs using the protocol necessary to communicate with the TinyWebDB component of an App Inventor app.
### - Send mails

### Author: David Wolber (wolber@usfca.edu), using sample of Hal Abelson

################## SETTINGS ##################
##Your Email Address This must be an admin for the web app on app engine
EmailFrom = "Matthew Davis<matthew.davis.1978@gmail.com>" #put email here make sure you keep the structure of name <(emailAddress)>
AuthKey = "caravaggio1" #put an auth key here just a random string and copy it into the Blocks editor
################## SETTINGS ##################
...
class sendemail(webapp.RequestHandler):
  ...
  def post(self):
    logging.info("GetValue POST Client IP: "+self.request.remote_addr)
    data = self.request.body
    self.process_email(data)

  def get(self):
    logging.info("GetValue GET Client IP: "+self.request.remote_addr)
