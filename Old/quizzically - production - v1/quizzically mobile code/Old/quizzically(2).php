	...
	if($data[0] == $SQLKEY) {  
		...
		if(get_magic_quotes_gpc()) {     
		    ...
		    if($type == "quiz") {
				...
				//Set as participant
				$query4 = sprintf("INSERT INTO P_PARTICIPANTS SELECT null, a.USE_ID, b.QUIZ_ID, 0, 0, null FROM (SELECT USE_ID FROM P_USERS WHERE USE_EMAIL = '%s') a, (SELECT QUIZ_ID FROM P_QUIZ WHERE QUIZ_CODE = '%s') b", $UserEmail, $QuizCode);
			    ...
			    $query = sprintf("SELECT a.QUIZ_CURRENT, a.QUIZ_STATUS, CAST( b.PART_SCORE AS CHAR ) AS PART_SCORE, d.QUESTION, d.QUEST_CATEGORY, e.ANS_A, e.ANS_B, e.ANS_C, e.ANS_D, e.ANS_CORRECT, a.QUIZ_CODE, a.QUEST_ID
			    ...
			    $query1 = sprintf("UPDATE P_PARTICIPANTS a, P_USERS b, P_QUIZ c
									...
									AND b.USE_EMAIL = '%s'
									AND c.QUIZ_CODE = '%s'", $score, $score, $user, $quiz);
				
				$query2 = sprintf("SELECT a.PART_LAST_SCORE 
